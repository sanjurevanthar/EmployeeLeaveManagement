...
sap.ui.define([
  ...
  return Controller.extend("app.controller.Manager", {
    ...
    onInit: function () {
      ...
      // optional debug to verify services are loaded (remove in production)
      // console.log("ApprovalRequestService:", ApprovalRequestService);
      // console.log("RejectionRequestService:", RejectionRequestService);
    },
    ...
    onRejectPress: async function (oEvent) {
      ...
      // CHECK context first (prevents reading getObject() on null)
      if (!oContext) {
        MessageBox.error("Failed to get item context.");
        console.error("Context is null.");
        return;
      ...
      try {
        // Confirmation Box (wraps callback in Promise)
        ...
        // Defensive check that service exists
        if (!RejectionRequestService || typeof RejectionRequestService.rejectionRequestService !== "function") {
          MessageBox.error("Reject service not available.");
          console.error("RejectionRequestService missing or invalid:", RejectionRequestService);
          return;
        }
        ...
        // Call is successful -> update the UI (remove the item)
        let aItems = oModel.getProperty("/items") || [];
        aItems = aItems.filter(o => o.LeaveID !== oItem.LeaveID);
        oModel.setProperty("/items", aItems);
        MessageBox.success("Request rejected successfully");
    ...
    onAcceptPress: async function (oEvent) {
      ...
      try {
        ...
        // Defensive check that service exists
        if (!ApprovalRequestService || typeof ApprovalRequestService.approvalRequestService !== "function") {
          MessageBox.error("Approve service not available.");
          console.error("ApprovalRequestService missing or invalid:", ApprovalRequestService);
          return;
        }
    ...
    // Back button in header — recreate the App view (go back)
    onBackPress: function () {
      if (window.navigationVariable) {
        window.navigationVariable("App");
      } else {
        console.error("window.navigationVariable is not defined");
      }
    }
